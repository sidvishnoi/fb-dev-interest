...
fbDevInterest.BASE_API_URL = `https://graph.facebook.com/v2.11/GROUPID/?&access_token=${fbDevInterest.API_KEY}&fields=name,feed.since(SINCE){message,updated_time,created_time,id,name,from,permalink_url},id,updated_time`;
...
fbDevInterest.getGroupFeed = function(options) {
  ...
  fetch(fetchUrl)
    .then((res) => res.json())
    .then(json => {
      if (!json.feed) throw `failed to fetch: ${fetchUrl}`;
